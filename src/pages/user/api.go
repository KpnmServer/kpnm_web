...
func UserHeadApi(ctx iris.Context){
	...
	file := USER_DATA_FOLDER.Folder(userid).File("head.png")
	ctx.ServeFile(file.Path())
}
...
func SendVerifyEmailApi(ctx iris.Context){
	...
	kses.DelSession(uid, "verify_flag")
	ctx.JSON(iris.Map{"status": "ok"})
}
...
func UserRegisterApi(ctx iris.Context){
	...
	USER_DATA_FOLDER.Folder(user.Id.String()).Create()
	ctx.JSON(iris.Map{"status": "ok"})
}
...
func UserRegcheckApi(ctx iris.Context){
	...
	if n, _ := kuser.USER_SQL_TABLE.Count(kpsql.OptWMapEq("email", email), kpsql.OptLimit(1)); n == 1 {
		ctx.JSON(iris.Map{
			"status": "error",
	...
	if n, _ := kuser.USER_SQL_TABLE.Count(kpsql.OptWMapEq("username", name), kpsql.OptLimit(1)); n == 1 {
		ctx.JSON(iris.Map{
			"status": "error",
			"error": "UserExistError",
